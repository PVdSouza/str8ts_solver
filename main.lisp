...
(defun solve (index board values colors)
    (if (= index 36)
        ...
        (if (= index 35)
            (if (eq values '())
                '()
                (if (isFinished colors (try 35 board (car values)))
                    (try 35 board (car values))
                    (solve 35 board (cdr values) colors)
                )
            )
            (if (null values)
                '()
                (if (eq (tryNext index board (car values) colors) '())
                    (solve index (try index board 0) (cdr values) colors)
...
(defun fullsolve()
    (solve (isValid 0 colorBoard) numberBoard (getOptions (isValid 0 colorBoard) numberBoard) colorBoard)
)